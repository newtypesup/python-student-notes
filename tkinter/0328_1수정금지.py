...
def search_blocks(starting_point, destination, blockchains, previous_hashes):
    ...
    while current_block != dest_block:
        ...
        if current_block.data1 != '"크로스 체인"':
            # 현재 블록 정보 출력
            print(f"현재 역: {current_block.data1}, 현재 노선: {current_blockchain}")
        ...
        if current_block.data1 != '"크로스 체인"':
            ...
            print(f"환승 이동: {current_blockchain} -> {next_blockchain}")
            print_block_info(current_block)

            current_blockchain = next_blockchain

    # 도착지 블록 정보 출력
    print(f"도착지에 도착했습니다: {destination}")
...
#   이미지 로드 및 표시
    ...
    image_resized = image_pil.resize((925, 600), Image.BICUBIC)  # 이미지 크기 조정
    ...
    label_start.place(relx=0.4, rely=0.85, anchor="center")
    entry_start = tk.Entry(window)
    entry_start.place(relx=0.5, rely=0.85, anchor="center")

    label_dest = tk.Label(window, text="목적지:")
    label_dest.place(relx=0.4, rely=0.9, anchor="center")
    entry_dest = tk.Entry(window)
    entry_dest.place(relx=0.5, rely=0.9, anchor="center")
