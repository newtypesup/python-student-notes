...
def on_search(blockchains, previous_hashes):
    ...
    distance_forward = search_blocks(start_point, destination, blockchains, previous_hashes)
    distance_reverse = search_blocks(destination, start_point, blockchains, previous_hashes)


    # 출발지와 도착지를 바꾼 후 다시 계산
    start_point, destination = destination, start_point  # 출발지와 도착지 교체
    start_block, dest_block = dest_block, start_block  # 출발지와 도착지 블록 교체
    distance_forward = search_blocks(start_point, destination, blockchains, previous_hashes)
    
    print(f"출발지: {start_point}, 도착지: {destination}")
    print(f"총 이동 거리: {distance_forward:.4f} km")
    print("출발지 정보:")
